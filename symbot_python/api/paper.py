"""Paper trading control surface — real live Bybit market data, simulated
fills, no real money at risk (PaperExchangeClient can never reach an
authenticated Bybit endpoint). Bots/deals live in-process only; there is
no persistence yet, so restarting the server clears all paper state.

There is no manual "type in your own DCA parameters" form any more —
paper trading always runs the continuous optimizer's current best
winner for ETHUSDT (see scripts/continuous_optimizer.py /
strategy/optimization_store.py), auto-refreshed every PARAM_REFRESH_SECONDS
(3x/day, i.e. every 8 hours), with a manual "Update to latest params now" button for an
immediate hot-apply. Typing in your own numbers here defeats the entire
point of running a continuous search: whatever the search just found is
what should actually be running, automatically, not something you have
to notice and copy over by hand.
"""
...
SYMBOL = "ETHUSDT"
PAIR = "ETH/USDT"
AUTO_BOT_NAME = f"paper-auto-{SYMBOL}"
PARAM_REFRESH_SECONDS = 24 * 3600 // 3  # "params should update in paper 3 times a day"
...
# Session telemetry baseline — set once, the moment the paper wallet is
# actually created from your real starting balance. Everything on the
# dashboard (session P/L %, the equity curve) is measured from this
# point, not from an arbitrary zero — restarting the server starts a new
...
async def get_manager() -> DCABotManager:
    """Paper trading starts from your REAL available Bybit balance (read-only
    Keychain lookup, same as the continuous optimizer) — never an
    arbitrary fake wallet when real funds ARE available. Fetched once, at
    first use; the paper wallet then tracks simulated fills from that
    real starting point exactly like a real bot would from here. Falls
    back to FALLBACK_STARTING_BALANCE_USDT only if the real available
    ...
    if _manager is None:
        ...
        if starting_balance < MIN_USABLE_BALANCE_USDT:
            logger.warning(
                "Real available balance is %.8f USDT — paper trading would never be "
                "able to open a deal. Falling back to a simulated %.2f USDT starting "
                "balance instead (no real funds involved).",
                starting_balance, FALLBACK_STARTING_BALANCE_USDT,
            )
            starting_balance = FALLBACK_STARTING_BALANCE_USDT
        initial_balances = {"USDT": starting_balance}
...
async def shutdown_manager() -> None:
    ...
    if _manager is not None:
        ...
        if close is not None:
            # PaperExchangeClient.close() briefly blocks (pybit's own
            # websocket exit() busy-waits for the socket to actually
            # close) — run it off the event loop rather than await it.
...
async def paper_status():
    """Trade counts used only to refresh the local paper dashboard."""
    manager = await get_manager()
    deals = list(manager.deals.values())
    return {
        "total_trades": sum(d.status == DealStatus.CLOSED for d in deals),
        "open_deal_count": sum(d.status == DealStatus.ACTIVE for d in deals),
    }


@router.post("/paper/update-params")
...
@router.post("/paper/clear-circuit-breaker")
...
@router.post("/paper/deals/{deal_id}/cancel")
...
@router.post("/paper/deals/{deal_id}/panic")
